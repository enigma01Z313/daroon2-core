...
function daroon2_theme_bookly_login_shortcode()
{
    ...
    if (is_user_logged_in()) {
        ...
        if (isset($_GET['complete_profile']) && isset($_GET['user_id'])) {
            ...
            if (isset($_POST['complete_profile_submit'])) {
                $full_name = sanitize_text_field($_POST['full_name']);
                $birth_date = sanitize_text_field($_POST['birth_date']);
                
                update_user_meta($current_user->ID, 'full_name', $full_name);
                update_user_meta($current_user->ID, 'birth_date', $birth_date);

                // Get the stored final redirect URL
                if (!session_id()) {
                    session_start();
                }

                unset($_SESSION['final_redirect_url']);
                wp_redirect($current_url. '?goto=therapist-bookly-form');
                exit;
            }
            ...
            ob_start();
            ?>
            <div class="login-form-container w-100 d-flex flex-wrap pt-8 mt-2" style="max-width: 350px;margin-left: auto; margin-right: auto;">
                <h1 class="title3 mb-3 w-100 text-center">Complete Your Profile</h1>
                <p class="body1 color-content-secondary mb-5 w-100 text-center">Please provide a few more details to complete your profile.</p>
                
                <form class="w-100 mb-5 d-flex flex-wrap justify-content-center" method="post">
                    <div class="mb-5 w-100">
                        <label class="title2 mb-1 pl-3 pr-3" for="full_name" style="display: block;">Full Name</label>
                        <input type="text" name="full_name" id="full_name" placeholder="Enter your full name" class="input w-100 pt-1 pb-1 pr-2 pl-2 body2" required>
                    </div>

                    <div class="mb-5 w-100">
                        <label class="title2 mb-1 pl-3 pr-3" for="birth_date" style="display: block;">Date of birth</label>
                        <input type="date" name="birth_date" id="birth_date" class="input w-100 pt-1 pb-1 pr-2 pl-2 body2" required>
                    </div>

                    <button type="submit" name="complete_profile_submit" class="btn btn-style-black btn-size-l w-100">Complete Profile</button>
                </form>
        ...
        if ($stored_otp && $submitted_otp == $stored_otp) {
            ...
            if ($user) {
                ...
                if (!$has_full_name && isset($_POST['full_name']) && isset($_POST['birth_date'])) {
                    ...
                    wp_redirect($current_url . (isset($_GET['goto']) ? '?goto=' . $_GET['goto'] : ''));
                    exit;
    ...
    <div class="login-form-container w-100 d-flex flex-wrap pt-8" style="max-width: 350px;margin-left: auto; margin-right: auto;">
        <?php if ($error) : ?>
        ...
        <?php if (!$success) : ?>
            <form method="post" id="login-form" class="mb-1 w-100 d-flex flex-wrap justify-content-center" action="<?php echo $current_url; ?>?goto=therapist-bookly-form">
                <div class="mb-2 w-100">
                    <label for="user_email" class="title2 mb-1 w-100 pr-3 pl-3" style="display: block;">Email Address</label>
                    <input type="email" name="user_email" id="user_email" class="input w-100 pt-1 pb-1 pr-2 pl-2 body2" placeholder="Email Address" required>
                </div>
                ...
                <button type="submit" name="login_submit" class="btn btn-style-black btn-size-l w-100">Sign in/ Sign up</button>
            </form>

            <p class="title1 color-content-secondary mb-1 w-100 text-center">Or</p>

            <?php echo daroon2_add_google_login_button('Sign in/ Sign up with Google'); ?>
        <?php elseif ($success === 'verify_success') : ?>
            ...
            <form class="w-100 mb-5 d-flex flex-wrap justify-content-center" method="post" id="profile-form">
                <div class="mb-5 w-100">
                    <label class="title2 mb-1 pl-3 pr-3" for="full_name" style="display: block;">Full Name</label>
                    <input type="text" name="full_name" id="full_name" placeholder="Enter your full name" class="input w-100 p-1 body2" required>
                </div>

                <div class="mb-5 w-100">
                    <label class="title2 mb-1 pl-3 pr-3" for="birth_date" style="display: block;">Date of birth</label>
                    <input type="date" name="birth_date" id="birth_date" class="input w-100 p-1 body2" required>
                </div>
            ...
            <p class="body1 color-content-secondary w-100 text-center">We use this information to personalize your experience and keep your account secure.</p>
        <?php else : ?>
            <div class="w-100 d-flex mt-2 mb-5">
                <a href="<?= $current_url; ?>?goto=therapist-bookly-form" class="btn btn-style-plain btn-size-l">
            ...
            <form class="w-100 mb-5 d-flex flex-wrap justify-content-center" method="post" id="otp-form">
                <div class="mb-5 w-100">
                    ...
                    <input type="text" name="otp" id="otp" placeholder="e.g. 123456" class="input w-100 p-1 body2" pattern="[0-9]{6}" maxlength="6" inputmode="numeric" required>
                </div>
            ...
            <p class="title1 color-content-secondary w-100 text-center color-content-secondary">
                Didn't receive the code? <a href="#" id="resend-link" class="color-content-primary" onclick="resendOTP(event, '<?php echo esc_attr($_POST['user_email']); ?>')">Resend code</a> <span id="timer" class="color-content-secondary"></span>
                <script>
                let resendTimer;
                let remainingTime = 0;
    ...
    <style>
        ...
        .google-login-container .btn span{
            --padding-x-count: 2
        }
...
add_shortcode('daroon2_theme_bookly_login', 'daroon2_theme_bookly_login_shortcode');
