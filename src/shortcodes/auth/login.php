...
function daroon2_theme_login_shortcode()
{
    ...
    if (is_user_logged_in()) {
        ...
        if (isset($_GET['complete_profile']) && isset($_GET['user_id'])) {
            ...
            if (isset($_POST['complete_profile_submit'])) {
                ...
                $redirect_url = isset($_SESSION['final_redirect_url']) ? $_SESSION['final_redirect_url'] : home_url('/account');
    ...
    if (isset($_POST['verify_otp'])) {
        ...
        if ($stored_otp && $submitted_otp == $stored_otp) {
            ...
            if ($user) {
                ...
                if (isset($_POST['full_name']) && isset($_POST['birth_date'])) {
                    ...
                    // Existing user with full name - just log them in
                    wp_set_current_user($user->ID);
                    wp_set_auth_cookie($user->ID, $remember);
                    do_action('wp_login', $user->user_login, $user);
    ...
    <div class="login-form-container w-100 d-flex flex-wrap pt-4" style="margin-left: auto; margin-right: auto;">
        ...
        <?php if (!$success) : ?>
            <h1 class="title3 mb-5 w-100 text-center">Sign in</h1>
            ...
            <form method="post" id="login-form" class="mb-2 w-100 d-flex flex-wrap justify-content-center">
                ...
                <button type="submit" name="login_submit" class="btn btn-style-black btn-size-l w-100">Sign in</button>
            </form>

            <p class="title1 color-content-secondary mb-2 w-100 text-center">Or</p>

            <?php echo daroon2_add_google_login_button(); ?>

            <p class="title1 color-content-secondary mb-6 w-100 text-center">
                Don't have an account? <a class="color-action-ember" href="<?php echo home_url(); ?>/signup" style="text-decoration: none;">Sign up</a>
            </p>
        ...
        <?php else : ?>
            <div class="w-100 d-flex mb-5">
                <a href="<?php echo home_url(); ?>/signin" class="btn btn-style-plain btn-size-l">
